...
describe('store', () => {
  ...
  it('playback mutations do not recompute the sim', () => {
    ...
    expect(s.get().overlays.v).toBe(false)
  })
  it('patchOrbits mutates orbits, recomputes sim, resets playback', () => {
    const s = createStore()
    s.setPlaying(true)
    s.setT(1)
    const before = s.get().sim
    s.patchOrbits({ vel: v(0, 1.2) })
    expect(s.get().orbits.vel).toEqual(v(0, 1.2))
    expect(s.get().sim).not.toBe(before)
    expect(s.get().playback.playing).toBe(false)
    expect(s.get().playback.t).toBe(0)
  })
  it('setTestCharge mutates the charges slice, recomputes sim, resets playback', () => {
    const s = createStore()
    s.setPlaying(true)
    s.setT(1)
    const before = s.get().sim
    s.setTestCharge({ testSign: -1 })
    expect(s.get().charges.testSign).toBe(-1)
    expect(s.get().sim).not.toBe(before)
    expect(s.get().playback.playing).toBe(false)
    expect(s.get().playback.t).toBe(0)
  })
  it('moveCharge mutates the charge position, recomputes sim, resets playback', () => {
    ...
    s.setT(1)
    const before = s.get().sim
    s.moveCharge(0, v(2, 2))
    expect(s.get().charges.charges[0].pos).toEqual(v(2, 2))
    expect(s.get().sim).not.toBe(before)
    expect(s.get().playback.playing).toBe(false)
    expect(s.get().playback.t).toBe(0)
  })
